...
def side_total(state:KalahState,player):
    opponent_side_total = sum(state.state[:6]) if player == state.SOUTH else sum(state.state[7:13]) 
    player_side_total = sum(state.state[7:13]) if player == state.SOUTH else sum(state.state[:6])
